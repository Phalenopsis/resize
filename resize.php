...
/**
 * Function to resize a .jpg image in a (new) directory "images" . $newValue . / . $imageName . $newValue . "." . jpg
 *
...
function resizeImage(string $source, string $typeValue = "W", array $newValues = ["256"],  int $compression = 70): bool
{
  ...
  */
  ...
  foreach ($newValues as $newValue) {
    ...
    $newFileName = renameReziseFile($source, $newValue);
    /*
...
unset($sizes[0]); // on efface la première valeur (qui est le nom du fichier.php)
...
function renameReziseFile(string $source, int $newValue): string
{
  $directoryArray = explode("/", $source);
  $sourceArray = explode(".", $directoryArray[count($directoryArray) - 1]);
  unset($directoryArray[count($directoryArray) - 1]);
  $dir = "";
  foreach ($directoryArray as $directory) {
    $dir .= $directory . "/";
  }
  $saveDirectory = $dir . "images" . $newValue;
  echo $saveDirectory;
